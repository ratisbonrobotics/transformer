...
def create_adam_state(params, learning_rate=5e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-8):
...
def train_step(learnable_params, adam_state, inputs, labels, pos, mask, n_heads, scale, vocab_size, total_steps):
    ...
    learning_rate = jax.lax.cond(adam_state['step'] <= WARMUP_STEPS, lambda _: adam_state['learning_rate'] * (adam_state['step'] / WARMUP_STEPS), lambda _: adam_state['learning_rate'] * (1 - adam_state['step'] / total_steps) ** 3, None)
