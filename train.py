...
# Create the model
model = FSDP(LanguageModel(VOCAB_SIZE).to(f"cuda:{os.environ['RANK']}"), mixed_precision=MixedPrecision(param_dtype=torch.bfloat16, reduce_dtype=torch.bfloat16, buffer_dtype=torch.bfloat16), cpu_offload=CPUOffload(offload_params=True), auto_wrap_policy=functools.partial(size_based_auto_wrap_policy, min_num_params=2000))
