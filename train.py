...
def loss_fn(learnable_params, inputs, labels, mask, n_heads, vocab_size):
    logits = language_model(learnable_params, inputs, mask, n_heads)
...
def train_step(learnable_params, optimizer_state, inputs, labels, mask, n_heads, vocab_size):
    loss, grads = jax.value_and_grad(loss_fn)(learnable_params, inputs, labels, mask, n_heads, vocab_size)
...
for epoch in range(NUM_EPOCHS):
    ...
    with tqdm.tqdm(indices) as pbar:
        for batch_idx in pbar:
            ...
            learnable_params, optimizer_state, loss = jit_train_step(learnable_params, optimizer_state, device_batch_inputs, device_batch_labels, static_config['mask'], static_config["n_heads"], train_dataset.vocab_size)
