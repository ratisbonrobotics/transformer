...
def apply_sm3_optimizer(learnable_params, sm3_state, grads, warmup_steps, total_steps):
    ...
    learnable_params = jax.tree_util.tree_map(lambda p, g, avg: p - learning_rate * (sm3_state["momentum"] * avg + (1 - sm3_state["momentum"]) * g), learnable_params, grads, sm3_state["grad_avg"])
...
def apply_rmsprop_optimizer(learnable_params, rmsprop_state, grads, warmup_steps, total_steps):
    ...
    learnable_params = jax.tree_util.tree_map(lambda p, g, avg: p - learning_rate * g / (jax.numpy.sqrt(rmsprop_state["decay_rate"] * avg + (1 - rmsprop_state["decay_rate"]) * jax.numpy.square(g)) + rmsprop_state["epsilon"]), learnable_params, grads, rmsprop_state["grad_sq_avg"])
